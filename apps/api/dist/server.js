...
export function createApp() {
    ...
    app.post('/bookings', async (req, res) => {
        try {
            ...
            await query(`INSERT INTO funnel_events (user_id, session_id, event_type, metadata, created_at)
         VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP)`, [parsed.renterId, parsed.sessionId, 'booking_created', JSON.stringify({ bookingId: booking.id })]);
    ...
    app.post('/bookings/:id/cancel', async (req, res) => {
        try {
            ...
            const refundAmount = policyService.calculateRefund(booking.totalPrice, new Date(), booking.startDate, policy);
    ...
    app.post('/inspections', async (req, res) => {
        try {
            ...
            if (parsed.status === 'rejected' && parsed.damageClaim) {
                claim = await claimService.createClaim(parsed.bookingId, parsed.damageClaim.damageType, parsed.damageClaim.amount);
    ...
    app.post('/bookings/:id/complete', async (req, res) => {
        try {
            ...
            const settlement = await settlementService.createSettlement(carrier.providerId, booking.id, booking.totalPrice, platformFee, providerPayout);
            ...
            await costService.recordCostEntry(booking.id, 'logistics', policy.roundTripShipping * PLATFORM_LOGISTICS_COST_RATIO);
            res.json({
                success: true,
                settlement,
    ...
    app.post('/funnel/events', async (req, res) => {
        try {
            ...
            const parsed = schema.parse(req.body);
            await query(`INSERT INTO funnel_events (user_id, session_id, event_type, metadata, created_at)
         VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP)`, [parsed.userId, parsed.sessionId, parsed.eventType, JSON.stringify(parsed.metadata || {})]);

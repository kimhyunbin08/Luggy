...
import { BlobSASPermissions, StorageSharedKeyCredential, generateBlobSASQueryParameters, SASProtocol, } from '@azure/storage-blob';
function parseConnectionString(connectionString) {
    const parts = Object.fromEntries(connectionString.split(';').filter(Boolean).map((pair) => {
...
function containerForCategory(category) {
    if (category === 'intake') {
...
function signAzureUpload(category, fileExtension) {
    const creds = parseConnectionString(process.env.AZURE_STORAGE_CONNECTION_STRING);
    if (!creds) {
...
function signLocalUpload(category, fileExtension) {
...
export function consumeLocalUploadTicket(token) {
 ...
 */
export function signUpload(category, fileName) {

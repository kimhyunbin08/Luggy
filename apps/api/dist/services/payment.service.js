...
export const PAYMENT_WEBHOOK_SECRET = process.env.PAYMENT_WEBHOOK_SECRET || 'dev-payment-webhook-secret-change-me';
if (process.env.NODE_ENV !== 'test' && !process.env.PAYMENT_WEBHOOK_SECRET) {
    console.warn('[payment.service] PAYMENT_WEBHOOK_SECRET is not set; using an insecure development default.');
}
function mapRow(row) {
    return {
        id: row.id,
...
async function upsertPayment(bookingId, amount, depositAmount, status, paymentIntentId) {
    ...
    const result = await query(`INSERT INTO payments
     ...
     RETURNING *`, [uuidv4(), bookingId, amount, depositAmount ?? null, status, 'mock_card', paymentIntentId, idempotencyKey]);
    return mapRow(result.rows[0]);
}
/**
 ...
 */
export async function processPaymentWebhook(rawBody, signatureHeader) {
 ...
 */
export async function authorizeAndConfirmMockPayment(bookingId) {

...
export async function getKpiSnapshot() {
    const funnelResult = await query(`SELECT event_type, COUNT(DISTINCT COALESCE(session_id, id::text)) as sessions
     FROM funnel_events
    ...
    const bookingStatusResult = await query(`SELECT status, COUNT(*)::int as count FROM bookings GROUP BY status`);
    ...
    const carrierResult = await query(`SELECT COUNT(*) FILTER (WHERE opt_in_rentable) as opted_in, COUNT(*) as total FROM carriers`);
    const optedIn = Number(carrierResult.rows[0]?.opted_in || 0);
    const totalCarriers = Number(carrierResult.rows[0]?.total || 0);
    const disputeResult = await query(`SELECT COUNT(DISTINCT booking_id)::int as disputed_bookings FROM damage_claims`);
    const disputedBookings = Number(disputeResult.rows[0]?.disputed_bookings || 0);
    const contributionResult = await query(`SELECT AVG(s.platform_fee - COALESCE(costs.total_cost, 0)) as avg_contribution
     FROM settlements s
     LEFT JOIN (
    ...
    return {
        ...
        avgContributionProfitPerBooking: avgContributionRaw !== null && avgContributionRaw !== undefined ? Number(avgContributionRaw) : null,

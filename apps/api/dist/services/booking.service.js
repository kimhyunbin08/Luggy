...
export async function createBooking(renterId, carrierId, policyVersionId, startDate, endDate, totalPrice, idempotencyKey) {
...
export async function updateBookingStatus(bookingId, status) {
    await query('UPDATE bookings SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [status, bookingId]);
}
export async function updateDeliveryStatus(bookingId, deliveryStatus) {
    await query('UPDATE bookings SET delivery_status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [deliveryStatus, bookingId]);
}
export async function getRenterBookings(renterId) {
    const result = await query('SELECT * FROM bookings WHERE renter_id = $1 ORDER BY created_at DESC', [renterId]);
...
export async function checkCarrierConflict(carrierId, startDate, endDate, excludeBookingId) {

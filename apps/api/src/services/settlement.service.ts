...
function mapRow(row: any): Settlement {
  return {
    id: row.id,
...
export async function createSettlement(
  ...
  const result = await query(
    `INSERT INTO settlements (id, provider_id, booking_id, gross_amount, platform_fee, provider_payout, status, created_at)
     VALUES ($1, $2, $3, $4, $5, $6, 'approved', CURRENT_TIMESTAMP)
     ON CONFLICT (booking_id) DO NOTHING
     RETURNING *`,
    [uuidv4(), providerId, bookingId, grossAmount, platformFee, providerPayout]
  );

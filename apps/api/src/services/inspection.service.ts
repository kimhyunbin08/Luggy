...
import { Inspection, InspectionPhoto } from '../models/types.js';
...
export async function createInspection(
  ...
  const result = await query(
    `INSERT INTO inspections 
     ...
     RETURNING *`,
    [inspectionId, bookingId, inspectionType, 'pending', inspectorId, now, now]
  );
...
export async function uploadInspectionPhoto(
  ...
  const result = await query(
    `INSERT INTO inspection_photos (id, inspection_id, photo_url, uploaded_at)
     VALUES ($1, $2, $3, CURRENT_TIMESTAMP)
     RETURNING *`,
    [photoId, inspectionId, photoUrl]
  );
...
export async function getBookingInspections(bookingId: string): Promise<Inspection[]> {
  const result = await query(
    'SELECT * FROM inspections WHERE booking_id = $1 ORDER BY created_at DESC',
    [bookingId]
  );

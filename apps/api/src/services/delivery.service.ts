...
import { DeliveryOrder } from '../models/types.js';
...
export const DELIVERY_WEBHOOK_SECRET =
  process.env.DELIVERY_WEBHOOK_SECRET || 'dev-delivery-webhook-secret-change-me';

if (process.env.NODE_ENV !== 'test' && !process.env.DELIVERY_WEBHOOK_SECRET) {
  console.warn(
    '[delivery.service] DELIVERY_WEBHOOK_SECRET is not set; using an insecure development default.'
  );
}
...
function mapRow(row: any): DeliveryOrder {
  return {
    id: row.id,
...
export async function getDeliveryTimeline(bookingId: string): Promise<DeliveryOrder[]> {
  const result = await query(
    'SELECT * FROM delivery_orders WHERE booking_id = $1 ORDER BY created_at ASC',
    [bookingId]
  );
...
export async function processDeliveryWebhook(
  ...
  const event = JSON.parse(rawBody) as DeliveryWebhookPayload;
  ...
  await query(
    `INSERT INTO delivery_orders (id, booking_id, direction, status, created_at, updated_at)
     VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
    [uuidv4(), bookingId, direction, status]
  );
 ...
 */
export async function simulateDeliveryEvent(
  bookingId: string,
  direction: DeliveryDirection,
  status: DeliveryStatus
): Promise<{ deduped: boolean }> {
  const event: DeliveryWebhookPayload = {

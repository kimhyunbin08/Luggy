...
## 18. 4차 파일럿: 로그인 기반 앱 셸 통합 (맡기기·렌탈·지도·프로필·설정) (기술 요구사항)
`prd.md` §21 참조. §1~§17의 기존 요구사항은 그대로 유지하며(변경/삭제 없음), 이 라운드는
**UI 재구성 중심의 additive 변경**이다 — 신규 백엔드 로직/스키마는 최소화하고 기존
API(§16.3, §17.3)를 최대한 재사용한다.

### 18.1 기술 스택 추가
- 신규 프론트엔드 프레임워크/라우팅 라이브러리는 도입하지 않는다. 기존
   `apps/web/src/main.ts`의 상태 기반 렌더링 패턴(`type Tab = "rent" | "provider" |
   "ops" | "p2p"` + 현재 탭 상태 변수 + `render*Tab()` 함수 조합)을 그대로 확장한다 —
   `Tab` 유니온 타입에 `"profile"`, `"settings"`를 추가하고, 사용자 하단 탭 배열에서
   `"ops"`를 제외한다(기능 자체는 제거하지 않음, §18.4 참조).
- 로그인 세션 저장은 기존 `apps/web/src/p2p.ts`에서 쓰던 `localStorage` 토큰 저장
   방식을 그대로 앱 셸(`main.ts`) 레벨로 승격한다(신규 스토리지 방식 도입 없음).

### 18.2 데이터 요구사항 추가
- 신규 테이블/컬럼 없음. "프로필" 탭은 기존 API 응답을 재구성해서 보여주며, 서버 측
   데이터 모델 변경이 필요하지 않다.

### 18.3 API 요구사항 추가/변경
- 신규 엔드포인트 없음(원칙). "프로필" 탭은 기존 `GET /auth/me`(§16.3 #1), 기존
   예약/위탁 캐리어 조회 API(§7 MVP 세트), `GET /deals`(§16.3 #7, unreadCount 포함
   §17.3 #4)를 조합해서 사용한다.
- 단, 클라이언트가 "내가 맡긴 캐리어 목록"을 조회할 기존 엔드포인트가 없다면(§7 MVP
   세트에 Provider 소유 캐리어 목록 조회용 GET이 명시돼 있지 않음), 구현 시점에
   `GET /providers/{id}/carriers` 형태의 조회 전용 엔드포인트 추가가 필요할 수 있다 —
   이는 신규 비즈니스 로직이 아닌 기존 데이터의 조회 뷰이므로 additive 원칙에 위배되지
   않는다(정확한 필요 여부는 구현 착수 시 기존 코드 재확인 후 확정).

### 18.4 상태/라우팅 요구사항 추가
1. 운영자 콘솔(`ops`)은 사용자 하단 탭 배열에서 제외하되, 코드/라우트는 삭제하지
    않고 별도 진입 경로(예: 관리자 역할 확인 후에만 노출되는 별도 URL 쿼리 파라미터 또는
    숨김 경로)로 유지한다. 구체적 접근 방식(쿼리 파라미터 vs 역할 기반 라우트)은 구현
    착수 시 결정한다.
2. 비로그인 상태에서 "맡기기"/"프로필"/"설정" 탭 진입 시 로그인/회원가입 폼으로
    리디렉션(또는 인라인 표시)한다 — 기존 `p2p.ts`의 "로그인 필요" 가드 패턴(§16.3 관련
    로직)을 재사용한다.
3. 로그인 세션은 앱 셸 전역 상태로 관리되어 탭 이동 시 유지된다.

### 18.5 보안/개인정보 요구사항 추가
- 기존 §16.5 원칙(세션 토큰 서버 시크릿 서명, 30일 TTL, 비밀번호 scrypt 해시)을 그대로
   따르며 변경 없음. 신규 위협 모델 없음(기존 인증을 노출 범위만 확장).

### 18.6 테스트 시나리오 추가
1. **Unit:** 탭 상태 전이 로직(로그인 여부에 따른 탭 접근 가드), `ops` 탭이 사용자
    내비게이션 목록에 포함되지 않는지 검증
2. **Integration:** 비로그인 상태에서 맡기기/프로필/설정 진입 시 로그인 유도 확인,
    로그인 후 탭 이동 시 세션 유지 확인, "프로필" 탭이 기존 API 조합 결과를 정확히
    표시하는지 확인
3. **E2E 게이트(신규):**
    1. 비로그인 게스트가 렌탈/지도 탐색 가능하고, 맡기기 진입 시 로그인 화면으로
       안내되는지 확인
    2. 로그인 후 5개 탭을 순회하며 세션이 끊기지 않는지 확인
    3. "맡기기" 탭에서 플랫폼 위탁/직거래 등록 두 모드 중 선택해 기존 등록 플로우가
       정상 동작하는지 확인

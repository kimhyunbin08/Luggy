...
## 22. 실제 행정구역 기반 동네 선택 (시/도 - 시/군/구 - 읍/면/동) (신규 기능)
자유 텍스트로 "우리 동네"를 입력하던 방식은 객관적이지 않은(오탈자, 임의 표기 등) 위치 데이터를 만들 수 있어, 실제 대한민국 행정구역 데이터를 기반으로 한 3단계 캐스케이딩 드롭다운으로 대체했다.

### 22.1 데이터 소스
- `apps/web/public/kr-districts.js`: 정적 데이터 파일(약 49KB), `window.KR_DISTRICTS`에 `[[시도명, [[시군구명, [동/읍/면명, ...]], ...]], ...]` 구조로 로드된다.
- 출처: 행정안전부 표준 행정동 경계 데이터(vuski/admdongkor, 2024-10-01 기준)의 `adm_nm`/`sidonm`/`sggnm` 속성만 추출(좌표/geometry 제외)하여 17개 시/도, 252개 시/군/구, 3,554개 읍/면/동 전체를 포함한다.
- 별도 API 키나 네트워크 호출 없이 오프라인으로도 동작하며, `index.html`에서 `<script src="/kr-districts.js">`로 다른 앱 스크립트보다 먼저 로드된다.

### 22.2 프런트엔드 구현 (`apps/web/index.html`)
- 가입 위저드 2단계(`#signup-step-2`)와 캐리어 등록 폼(내 캐리어 등록하기)에 각각 `{prefix}-sido`/`{prefix}-sigungu`/`{prefix}-dong` 3개의 `<select>`와 최종 값을 담는 `<input type="hidden" id="{prefix}-district">`을 배치한다 (`prefix`는 `s`(가입)/`p`(캐리어 등록)).
- `initDistrictPicker(prefix)`: 페이지 로드시 시/도 옵션을 채운다.
- `onDistrictSidoChange(prefix)` → 시/군/구 옵션 활성화, `onDistrictSigunguChange(prefix)` → 읍/면/동 옵션 활성화, `onDistrictDongChange(prefix)`: 셋 다 선택되면 `updateDistrictHiddenValue(prefix)`가 배민/당근마켓 스타일의 "구 동" 형식(예: "강남구 역삼1동")으로 hidden input 값을 갱신한다. 시/도는 표시 형식에서 생략한다(기존 헤더 배지/카드 표기와 동일한 포맷 유지).
- 3단계 중 하나라도 선택되지 않으면 hidden 값은 빈 문자열이며, 가입 2단계 검증("우리 동네(시/군/구/동)를 모두 선택해주세요.")과 캐리어 등록 검증("거래 가능 동네(시/군/구/동)를 모두 선택해주세요.")에서 이를 차단한다.
- 기존 백엔드 `POST /auth/signup`/`POST /providers/carriers`의 `district` 필드 계약은 변경되지 않는다(여전히 "구 동" 형식의 문자열 하나를 받는다) — 프런트엔드에서만 입력 방식이 자유 텍스트에서 실제 주소 캐스케이딩 선택으로 바뀌었다.

### 22.3 테스트 시나리오 (신규 기능)
#### E2E 테스트 (수동/브라우저 캔버스로 검증)
1. 가입 위저드 2단계 진입 시 시/도 드롭다운에 17개 항목(서울특별시~제주특별자치도)이 모두 나열되는지 확인
2. 시/도 선택 시 해당 시/도의 시/군/구 목록만 활성화되어 나열되는지 확인(예: "서울특별시" 선택 시 "강남구" 등 25개 구)
3. 시/군/구 선택 시 해당 구의 읍/면/동 목록만 활성화되어 나열되는지 확인(예: "강남구" 선택 시 "역삼1동" 등 22개 동)
4. 시/도만 선택하고 시/군/구·동을 선택하지 않은 채 "다음"을 누르면 진행이 차단되는지 확인
5. 시/군/구·동까지 모두 선택 후 가입 완료 시 헤더 배지에 "구 동" 형식으로 정확히 표시되는지 확인
6. 캐리어 등록 폼에서도 동일한 3단계 선택 UI가 동작하고, 미선택 시 등록이 차단되는지 확인

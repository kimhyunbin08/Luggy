...
## 4-5) 4차 파일럿 In Scope — 로그인 기반 앱 셸 통합 (맡기기·렌탈·지도·프로필·설정)
기존 3개 라운드(§3, §4-1, §4-3)에서 파편적으로 흩어진 기능을 하나의 로그인 기반 유저
플로우로 재구성하는 라운드다. 신규 비즈니스 로직 추가가 아닌 **정보구조(IA) 재배치**가
핵심이며, 상세 배경/정책은 `ideation.md` §15, `prd.md` §21, `trd.md` §18 참조.
- 전역 로그인/회원가입(§4-1의 이메일/비밀번호 인증을 앱 전체로 확장, 신규 인증 수단 추가 없음)
- 5-tab 하단 내비게이션: 맡기기(저장)/렌탈/지도/프로필/설정
- "맡기기" 탭에 플랫폼 위탁(legacy)과 P2P 직거래 등록을 하위 모드로 통합(백엔드 로직 변경 없음)
- "프로필" 탭(신규): 내 캐리어/내 예약/내 P2P 요청함·채팅(unreadCount 포함) 조회
- "설정" 탭(신규): 로그아웃, 계정 정보 읽기 전용 표시
- 게스트 모드 유지(렌탈/지도 탐색은 비로그인 가능)
- 운영자 콘솔(`ops`)은 사용자 하단 탭에서 제외, 별도 경로로 분리(기능 자체는 유지)

### 4-6) 4차 파일럿 Out of Scope (Deferred, TODO/플래그만 유지)
- 카카오 로그인 등 소셜 로그인 추가
- 프로필/계정 정보 수정(이름/전화번호/비밀번호 변경)
- 설정 탭의 알림 환경설정 등 부가 기능
- 온보딩 튜토리얼
...
### 7-3) 4차 파일럿 API 추가 세트 (로그인 기반 앱 셸 통합)
- 원칙적으로 신규 엔드포인트 없음(기존 §7-1/§7-2 API 재사용, UI 재구성 중심)
- "내가 맡긴 캐리어 목록" 조회용 엔드포인트가 없다면 구현 시 조회 전용으로만 추가 검토
  (`trd.md` §18.3)
...
- 3차 파일럿 게이트(신규): 반경 필터 거리순 정렬, 실시간 채팅 즉시 반영(폴링 없이), 약속
  제안 메시지 UI 구분 표시
- 4차 파일럿 게이트(신규): 비로그인 게스트 렌탈/지도 탐색, 로그인 후 5개 탭 세션 유지,
  맡기기 탭 내 플랫폼/직거래 모드 선택 동작
...
- 4차 파일럿(로그인 기반 앱 셸 통합)도 **추가(additive)**로만 구현하며, 기존 라운드(§3,
  §4-1, §4-3)의 코드/스키마/E2E 게이트를 깨뜨리지 않는다. 신규 비즈니스 로직보다 기존
  기능의 재배치를 우선하며, **사용자가 명시적으로 구현을 지시하기 전까지 코드를 작성하지
  않는다**(문서 확정 우선).

...
- **Design System:** OTA 밀도형 비교 리스트 UI를 기준으로, 배경은 순백 대신 옅은 웜/쿨 그레이를 사용한다.
- **Design System:** 프라이머리는 딥 네이비/딥 그린, CTA·가격 강조는 고채도 코랄/오렌지/레드의 단색 솔리드만 허용한다.
- **Design System:** 배지 색상은 목적별로 분리한다(검수완료=그린, 희소성/긴급=레드, 혜택=옐로우/보조색, 등급=네이비/골드).
- **Design System:** 레이아웃은 상단 sticky 검색바 + 좌측 필터 레일 + 우측 결과 리스트, 상세는 좌측 정보 + 우측 sticky 예약 위젯을 기본 패턴으로 한다.
- **Design System:** 카드당 핵심 정보(썸네일, 브랜드/모델, 평점·리뷰, 검수/희소성 배지, 원가·총결제액, 도착예정)를 동시에 노출하는 고밀도 원칙을 따른다.
